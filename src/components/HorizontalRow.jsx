...
export default function HorizontalRow({ title, fetchFn, onMovieClick, initialMovies, initialTotalPages = 1 }) {
  const hasInitialMovies = Array.isArray(initialMovies) && initialMovies.length > 0;
  const [movies, setMovies] = useState(hasInitialMovies ? initialMovies : []);
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(!hasInitialMovies);
  const [hasMore, setHasMore] = useState(hasInitialMovies ? 1 < Math.min(initialTotalPages || 1, 10) : true);
  ...
  useEffect(() => {
    ...
    if (hasInitialMovies) {
      ...
      setHasMore(1 < Math.min(initialTotalPages || 1, 10));
      setLoading(false);
      return;
    }
    ...
    setHasMore(true);
    load(1);
  }, [load, hasInitialMovies, initialMovies, initialTotalPages]);
