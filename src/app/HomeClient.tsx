...
import "../App.css";
import { getByGenre, NAV_CATEGORIES, ROWS } from "../tmdb";
...
export default function HomeClient({
  ...
  initialPopularTotalPages,
}: {
  initialPopularMovies: Movie[];
  initialPopularTotalPages: number;
}) {
  const [activeCat, setActiveCat] = useState("all");
  const [selectedMovie, setSelectedMovie] = useState<Movie | null>(null);
  const [extraRows, setExtraRows] = useState<MovieRow[]>([]);
  ...
  const currentRows = useMemo(() => CAT_ROWS[activeCat as keyof typeof CAT_ROWS] || ROWS, [activeCat]);
